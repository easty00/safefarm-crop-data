"""crop_disaster_rules — 작물별 재해 경보 기준. **Safe-farm 에 넣을 제안이다.**

이 파일은 이 폴더에서 돌지 않는다. 옮겨 넣을 자리는 —

    Safe-farm/ai-service/app/models/farm/crop_disaster_rule.py

같이 고쳐야 할 곳 —

    app/models/farm/__init__.py                이 모델을 import
    pipeline/farm/master_seed_farm_db.py       TABLES 에 "crop_disaster_rules" 추가
                                               UNIQUE·REFS 목록에도 한 줄씩
    data/dummy/crop_disaster_rules.csv         api테스트/pipeline/out/스키마/ 에서 복사

─────────────────────────────────────────────────────────────────────
⚠ 저쪽에 이미 `disaster_rules` 가 있는데 **다른 것이다. 섞지 말 것.**

    disaster_rules        station · risk("01".."05") · solar_term · ta_min · tg_min
                          "절기재해 기준값(다년 평균)" — 관측소별 기상 통계
                          app.core.db.Base (문서·기상 계열)
...
★ 지금 이 값들은 프런트에 상수로 박혀 있다.
    ...
    src/features/growth/domain/growthStage.ts
        상추   frostRiskBelowC: 4  · heatRiskAboveC: 26
        토마토  frostRiskBelowC: 6  · heatRiskAboveC: 32

    작물당 하나씩이라 **생육단계와 등급을 못 담는다.** 이 표는 그 둘을 담는다.
    고추 개화착과기는 30℃ 주의 · 32℃ 위험처럼 갈린다.

─────────────────────────────────────────────────────────────────────
값의 출처

    농촌진흥청 농사로 API — 재해예방정보 · 주간농사정보 · 농작업일정
    52행 · 10작물. 근거(실린 호 수 · 원문)는 api테스트/pipeline/out/스키마/근거/ 에 있다
"""
...
class CropDisasterRule(FarmBase):
    ...
    __table_args__ = (
        ...
        #   저쪽 disaster_rules.crop_id 가 같은 까닭으로 그렇게 하고 있다
        UniqueConstraint(
