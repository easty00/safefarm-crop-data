...
# 숫자 **바로 앞**에 붙은 라벨. 앞 창(앞 14자)의 끝에 고정($)해서 "이 숫자의 라벨" 만 본다 —
# 같은 줄 다른 값에 새지 않는다(위 ⚠ 의 25~30 사고를 다시 안 내려고).
#
# ⚠ 국내 농작업일정은 기준·상한을 **괄호 안에 맨 '최저'·'최고'** 로 적는다.
#     ■ 생육온도 : 최적 25℃ (최 저 3~4.5℃, 최고 30~32℃)      밀
#     ■ 생육적온 : 25~27℃ (최저 15℃)                         땅콩
#   '최 저' 처럼 자간이 벌어진 것도 있다(hwpx). 그래서 최\s*저 다.
#   빈 115종 중 28종의 기준온도가 이 꼴이었다(2026-09-17 실측) — 접두·접미를 요구하던
#   옛 정규식은 맥주보리('생육최저온도 :')는 잡고 밀('(최 저')은 놓쳤다.
_붙음 = r"(?:\s*(?:온도|기온))?\s*[:：]?\s*[(（]?\s*$"
LOW_LABEL = re.compile(
    r"(생육\s*최저\s*온도|발아\s*최저\s*온도|생육\s*최저(?!\s*한계)|발아\s*최저(?!\s*한계)|최저\s*온도"
    r"|(?:생육|발아)?\s*최\s*저" + _붙음 + r")"
)
HIGH_LABEL = re.compile(
    r"(생육\s*최고\s*온도|발아\s*최고\s*온도|최고\s*온도"
    r"|(?:생육|발아)?\s*최\s*고" + _붙음 + r")"
)
OPT_LABEL = re.compile(
    r"(적\s*온|최적\s*온도|알맞은\s*온도"
    r"|최\s*적" + _붙음 + r")"
)
