...
# ⚠ 2026-09-18 에 `강·중·약` → `쉬움·보통·어려움` 으로 통일했다.
#   저쪽 프런트(cropOption.ts toDifficultyLevel)가 이 낱말만 알아본다 —
#   전에는 DB 에 `강·중·약` 이 들어가 13작물 전부가 "보통" 으로 그려지고 있었다.
#   저쪽 CheckConstraint 도 같이 바뀌어야 한다(ck_crops_difficulty).
_등급 = {"쉬움", "보통", "어려움"}
...
def 관리노력():
    ...
    ⚠ **재배 난이도가 아니라 '얼마나 자주 돌보나' 다.**
      어려움 = 거의 매일 · 보통 = 주 1~2회 · 쉬움 = 월 1~2회 (§H 정본).
      원문(『텃밭 디자인』 9쪽)은 `강·중·약` 이라고 적지만 저쪽 프런트가 쓰는
      낱말에 맞춰 옮겨 적는다.
    ...
    for r in 행들:
        ...
        if 등급 not in _등급:
            raise 확정표오류(
                f"{작물}.difficulty 가 '{등급}' 입니다 — 쉬움·보통·어려움 셋 중 하나여야 합니다"
            )
        out[작물] = {
            "difficulty": 등급,
            "confirmed": "Y" if re.sub(r"[*`\s]", "", r["confirmed"])[:1] == "Y" else "N",
...
def _파종일(글, 키):
    ...
    ⚠ **이 값을 건드리지 말 것.** gdd_backfill 이 여기서부터 열을 쌓아
      gdd_target 을 만든다. 하루만 밀려도 21개 목표값이 전부 바뀐다.
      화면에 보여줄 파종 창은 _파종창() 이 따로 만든다.
    """
    m = _파종순.search(글 or "")
    if not m:
...
def _파종창(글, 키):
    """'아주심기 8.중~9.상' → ('아주심기', '08-11', '09-10').

    사용자에게 "8월 중순부터 9월 상순까지" 를 보여주려면 중앙일 하나로는 부족하다.
    시작 순의 첫날부터 끝 순의 끝날까지가 실제로 심어도 되는 기간이다.

    순이 하나뿐이면('아주심기 5.상') 그 순 하나가 창이다 — 05-01~05-10.
    방법은 첫 순 앞의 글자다: 씨뿌림 · 아주심기 · 모내기 · 파종.
    """
    순들 = _파종순.findall(글 or "")
    if not 순들:
        raise 확정표오류(f"{키}: 파종·정식에서 순(旬)을 못 읽었습니다 — {글!r}")
    방법 = (글 or "")[: _파종순.search(글).start()].strip(" ·~")
    # ⚠ **굵게 표시를 벗긴다.** 확정표는 바뀐 값을 `**발아 3.하**` 처럼 굵게 적는데,
    #   그 별표가 `sow_method` 로 그대로 새어 나가 CSV 에 '**발아' 가 실렸다
    #   (참다래 · 2026-09-20 실측). 표기는 문서의 것이지 값의 일부가 아니다.
    방법 = 방법.replace("*", "").strip()
    return 방법, era_edge(*순들[0]), era_edge(*순들[-1], last=True)


def 작형():
    """[{작물, 작물키, 작형, 지역, 관측소, 파종일, 파종방법, 파종시작, 파종끝, 일수}].
    등록된 작물만.
    ...
    for r in 행들:
        ...
        out.append({
            ...
            "파종일": _파종일(r["파종·정식"], 키),
            # 위 파종일과 별개다. 저건 GDD 역산의 시작점, 이 셋은 화면에 보여줄 값
            "파종방법": 방법,
            "파종시작": 시작,
            "파종끝": 끝,
            "일수": _수(r["일수"], f"{키}.일수"),
